...
async function main() {
  ...
  if (profilesWithEmptyCollege.length > 0) {
    console.log(
      `Found ${profilesWithEmptyCollege.length} profiles with empty collegeId. Setting to null...`,
    );
  ...
  for (const profile of allProfiles) {
    ...
    if (!user) {
      console.log(
        `🗑️ Deleting orphan profile ${profile.id} (User ${profile.userId} not found)`,
      );
  ...
  if (invalidSteps > 0) {
    console.log(
      `Found ${invalidSteps} profiles with invalid onboardingStep. Resetting to 0...`,
    );
    await prisma.profile.updateMany({
      where: { onboardingStep: { lt: 0 } },
  ...
  if (!admin) {
    console.log(
      "⚠️ Platform Admin not found. Please register manually or seed.",
    );
  } else {
