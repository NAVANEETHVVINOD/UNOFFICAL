...
export class SavedService {
    ...
    async create(userId: string, createSavedDto: CreateSavedDto) {
        ...
        if (type === SavedType.POST && !postId) throw new BadRequestException('postId is required for POST type');
        if (type === SavedType.EVENT && !eventId) throw new BadRequestException('eventId is required for EVENT type');
        if (type === SavedType.LISTING && !listingId) throw new BadRequestException('listingId is required for LISTING type');
        if (type === SavedType.NOTE && !noteId) throw new BadRequestException('noteId is required for NOTE type');
