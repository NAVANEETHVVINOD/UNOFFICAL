        ...
        <div className="space-y-8">
                            ...
                            <div className="mb-8 group cursor-pointer">
                                <div className="h-64 bg-gray-200 rounded-xl mb-4 overflow-hidden relative border-2 border-black">
                                    <div className="absolute inset-0 bg-accent-blue/20 flex items-center justify-center">
                                        <Doodle src="/doodles/star.svg" className="w-32 h-32 opacity-50" />
                                    </div>
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                                    <Badge className="absolute bottom-4 left-4 bg-accent-pink text-white border-white">FEATURED EVENT</Badge>
                                </div>
                                ...
                                <p className="font-serif text-lg text-gray-600 leading-relaxed line-clamp-2">
                                    {upcomingEvents[0].description || "Join us for this amazing event!"}
                                </p>
                            </div>
                        ) : (
                            <div className="mb-8 text-center py-12 bg-gray-50 rounded-xl border-2 border-dashed border-gray-300">
                                <Doodle src="/doodles/sad-face.svg" className="w-16 h-16 mx-auto mb-4 opacity-50" />
                                <h3 className="font-bold text-xl">No News Yet</h3>
                                <p className="text-gray-500">Things are quiet on campus today.</p>
                            </div>
                        )}
                    ...
                    <NewspaperCard variant="curved" className="bg-accent-blue text-white p-6 relative overflow-hidden">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center border-b border-white/20 pb-2">
                                ...
                                <span className="font-pixel text-xl">{stats?.totalMembers || 0}</span>
                            </div>
                            <div className="flex justify-between items-center border-b border-white/20 pb-2">
                                <span className="font-serif italic">Clubs</span>
                                <span className="font-pixel text-xl">{stats?.totalClubs || 0}</span>
                            </div>
                            <div className="flex justify-between items-center border-b border-white/20 pb-2">
                                <span className="font-serif italic">Events</span>
                                <span className="font-pixel text-xl">{stats?.totalEvents || 0}</span>
                            </div>
                        </div>
                    </NewspaperCard>
            ...
            <div className="-mx-4 md:-mx-8 transform rotate-1">
                <div className="bg-accent-yellow py-3 border-y-4 border-black">
                    <Marquee speed={40}>
                        <span className="font-bold text-xl mx-8 uppercase">/// {collegeName} ///</span>
                        <span className="font-serif italic text-xl mx-8">Student Hub</span>
                        <span className="font-bold text-xl mx-8 uppercase">/// Events ///</span>
                        <span className="font-serif italic text-xl mx-8">Clubs</span>
                        <span className="font-bold text-xl mx-8 uppercase">/// Marketplace ///</span>
                    </Marquee>
                </div>
            </div>
                    ...
                    <div className="p-4 space-y-2">
                        {upcomingEvents.length > 0 ? (
                            upcomingEvents.map((event: any) => (
                                <EventRow
                                    key={event.id}
                                    date={new Date(event.startsAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }).toUpperCase()}
                                    title={event.title}
                                    time={new Date(event.startsAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                            ...
                            <div className="text-center py-8">
                                <p className="text-gray-500 font-serif italic">No upcoming events.</p>
                            </div>
                        )}
                    </div>
                </NewspaperCard>
                                ...
                                <Link key={club.id} href={`/clubs/${club.id}`}>
                                    <div className="bg-gray-50 p-4 rounded-xl border-2 border-black hover:-translate-y-1 transition-transform cursor-pointer h-full">
                                        <div className="w-10 h-10 bg-accent-purple rounded-full mb-3 border-2 border-black overflow-hidden">
                                            {club.logoUrl && <img src={club.logoUrl} alt={club.name} className="w-full h-full object-cover" />}
                                        </div>
                                        <h4 className="font-bold line-clamp-1">{club.name}</h4>
                                        <p className="text-xs text-gray-500 mt-1 line-clamp-2">{club.description || "Join us!"}</p>
                                    </div>
                                </Link>
                            ))
                        ) : (
                            <div className="col-span-2 text-center py-8">
                                <p className="text-gray-500 font-serif italic">No clubs found.</p>
                            </div>
                        )}
