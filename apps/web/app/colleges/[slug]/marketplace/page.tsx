...
export default function CollegeMarketplacePage({ params }: PageProps) {
    ...
    return (
        <div className="min-h-screen">
            ...
            >
                <div>
                    <h1 className="font-display text-4xl md:text-6xl font-black mb-2">MARKETPLACE</h1>
                        ...
                        >
                            <Link href={`/marketplace/${item.id}`}>
                                <NewspaperCard className="h-full hover:-translate-y-2 hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transition-all cursor-pointer group bg-white p-0 overflow-hidden flex flex-col relative">
                                    {/* Price Tag */}
                                    <Sticker className="top-2 right-2 bg-accent-green text-black font-bold z-10" rotate={5}>
                                        ${item.price}
                                    </Sticker>
                                    ...
                                    <div className="p-4 flex flex-col flex-grow">
                                        <h3 className="font-bold text-lg mb-1 leading-tight group-hover:underline decoration-2 decoration-accent-green line-clamp-1">{item.title}</h3>
            ...
            {!loading && items.length === 0 && (
                <div className="text-center py-20">
                    ...
                    <p className="text-gray-600">The market is empty. Time to declutter?</p>
                </div>
            )}
        </div>
    );
