...
export function HangingCard({
  ...
  return (
    <div
      className={`relative animate-swing ${className}`}
...
export function Marquee({
  ...
  return (
    <div className={`overflow-hidden whitespace-nowrap flex ${className}`}>
      <div
        className={`animate-marquee flex shrink-0 ${direction === "right" ? "animate-marquee-reverse" : ""}`}
...
export function RetroButton({
  ...
  const baseStyles =
    "px-8 py-3 font-bold text-sm uppercase tracking-wider border-2 border-black transition-all duration-200 active:translate-x-[2px] active:translate-y-[2px] active:shadow-none flex items-center justify-center gap-2 rounded-full disabled:opacity-50 disabled:cursor-not-allowed";

  const variants = {
    primary:
      "bg-black text-white shadow-[4px_4px_0px_0px_rgba(0,0,0,0.2)] hover:bg-gray-900",
    secondary:
...
export function Badge({
  ...
  return (
    <span
      className={`inline-block px-4 py-1.5 border-2 border-black text-xs font-bold bg-accent-yellow shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] rounded-full ${className}`}
    >
      {children}
    </span>
  );
...
export function Staple({ className = "" }: { className?: string }) {
  return (
    <div
      className={`w-4 h-10 border-l-2 border-r-2 border-t-2 border-gray-400 rounded-t-md bg-transparent absolute -top-5 left-1/2 transform -translate-x-1/2 z-20 ${className}`}
    />
  );
}
...
export function Tape({ className = "" }: { className?: string }) {
  return (
    <div
      className={`w-32 h-8 bg-accent-yellow/50 backdrop-blur-sm transform -rotate-3 border-l-2 border-black/10 absolute -top-3 left-1/2 -translate-x-1/2 z-10 ${className}`}
    />
  );
}
...
export function EventRow({
  ...
  return (
    <div className="flex items-center gap-6 py-6 border-b border-gray-100 hover:bg-gray-50 transition-colors group cursor-pointer px-6 first:rounded-t-[2rem] last:rounded-b-[2rem] last:border-b-0">
      <div
        className={`w-12 h-12 ${color} rounded-full flex items-center justify-center shrink-0 border border-black shadow-sm`}
      >
      ...
      <div className="flex-1">
        <h3 className="font-serif text-xl italic text-gray-800 group-hover:text-black transition-colors">
          {title}
        </h3>
      </div>
