 ...
 * 
 * A dashboard for working professionals that displays:
 * - Same layout as StudentDashboard
 * - Defaults to Global Events view instead of Campus Events
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        // Fetch global events (Requirement 5.2 - default to Global Events view)
        const globalData = await api.getEvents({ 
          scope: 'global',
          limit: 6 
        });
        const globalArray = globalData?.events || globalData || [];
        setGlobalEvents(Array.isArray(globalArray) ? globalArray : []);

        // Fetch trending/popular events
        const trendingData = await api.getEvents({ 
          scope: 'global',
          limit: 6 
        });
        const trendingArray = trendingData?.events || trendingData || [];
        setTrendingEvents(Array.isArray(trendingArray) ? trendingArray : []);
      } catch (error) {
      ...
      >
        ...
        <motion.div variants={itemVariants}>
          ...
          <p className="text-neutral-600 dark:text-dark-text-muted font-mono text-sm">
            Attend events and network globally
          </p>
        </motion.div>
        ...
        {/* Global Events Section - Primary focus for professionals (Requirement 5.2) */}
        ...
        {upcomingEvents.length === 0 && 
         ...
         trendingEvents.length === 0 && (
          ...
          >
            {/* Globe/network doodle illustration */}
            <div className="w-32 h-32 mx-auto mb-6 relative">
              <img
                src="/doodles/group.svg"
                alt="Globe and network illustration"
            ...
            <h3 className="font-display text-2xl font-black text-ink dark:text-dark-text mb-3">
              No events in your area yet — discover global opportunities
            </h3>
            <p className="text-neutral-500 dark:text-dark-text-muted mb-8 max-w-md mx-auto font-mono text-sm">
              Network with professionals worldwide and attend exciting events
            </p>
            <Link href="/events">
  ...
  return (
    ...
    >
      <Link href={`/events/${event.id}`}>
        ...
        <div className="h-40 bg-gradient-to-br from-accent-blue/20 to-accent-coral/20 dark:from-accent-blue/10 dark:to-accent-coral/10 relative overflow-hidden">
          {event.coverUrl ? (
            <img
              src={event.coverUrl}
              alt={event.title}
            ...
            <div className="w-full h-full flex items-center justify-center">
              <Calendar className="w-12 h-12 text-accent-blue/40 dark:text-accent-blue/20" />
            </div>
          )}
        ...
        <div className="p-4">
          ...
          {/* Action Buttons - Attendee Mode (Requirement 5.4) */}
