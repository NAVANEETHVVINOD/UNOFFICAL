...
import { PencilDoodle, BulbDoodle, HeartDoodle, StarDoodle } from "../doodles/CollegeDoodles"
...
const StoryCard = ({ story, index }: { story: typeof stories[0], index: number }) => (
  ...
  >
    <NotebookCard className={`transform ${index % 2 === 0 ? 'rotate-[-1deg]' : 'rotate-[1deg]'} relative`}>
      <div className="relative">
        ...
        >
          <NotebookText className="text-sm text-accent-blue">{story.impact}</NotebookText>
        </motion.div>
        <motion.div 
        ...
        >
          {index % 3 === 0 ? <PencilDoodle /> : index % 3 === 1 ? <HeartDoodle /> : <StarDoodle />}
        </motion.div>
      </div>
    </NotebookCard>
  </motion.div>
    ...
    <NotebookPage withMargin>
      <div className="max-w-7xl mx-auto px-4 py-20">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <NotebookTitle className="mb-4">Made by Students, for Students</NotebookTitle>
          <NotebookText className="text-xl max-w-2xl mx-auto">
            Real stories from our college community. Each story is a new page in our shared notebook.
          </NotebookText>
        </motion.div>
