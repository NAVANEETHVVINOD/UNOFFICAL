...
export default function ItemDetailsClient() {
    ...
    const handleContact = () => {
        ...
        alert(`Contacting ${item.owner.profile.fullName} at ${item.owner.email}`);
    };
        ...
        return (
            <Container>
                <div className="min-h-screen flex flex-col items-center justify-center text-center">
                    ...
                    <RetroButton onClick={() => router.push('/marketplace')}>GO BACK</RetroButton>
                </div>
            </Container>
        );
    }
                        ...
                        <div className="grid md:grid-cols-2 gap-8">
                                ...
                                <div className="bg-white border-4 border-black p-4 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] rotate-[-2deg]">
                                    <div className="aspect-square bg-gray-100 flex items-center justify-center overflow-hidden border-2 border-black">
                                        {item.imageUrl ? (
                                            ...
                                            <Doodle src="/doodles/camera.svg" className="w-32 h-32 opacity-20" />
                                        )}
                                    </div>
                            ...
                            >
                                <div>
                                    ...
                                    <h1 className="font-display text-4xl md:text-5xl font-black mb-4 leading-tight">{item.title}</h1>
                                    <div className="flex items-center gap-2 mb-6">
                                        <span className={`px-3 py-1 text-xs font-bold border-2 border-black ${item.status === 'ACTIVE' ? 'bg-green-400' : 'bg-red-400'}`}>
                                            {item.status}
                                        </span>
                                ...
                                <NewspaperCard className="p-6 bg-white">
                                    <h3 className="font-bold text-lg uppercase mb-4 border-b-2 border-black pb-2">Description</h3>
                                ...
                                <NewspaperCard className="p-6 bg-accent-blue/10">
                                    <h3 className="font-bold text-lg uppercase mb-4 border-b-2 border-black pb-2">Seller Info</h3>
                                    <div className="flex items-center gap-4 mb-4">
                                        ...
                                        <div>
                                            <p className="font-bold text-lg">{item.owner.profile.fullName}</p>
                                            <p className="font-mono text-xs text-gray-600">Student ID: {item.owner.profile.collegeId || 'N/A'}</p>
                                        </div>
                                    </div>
